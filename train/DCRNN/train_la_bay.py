...
from model.Backbone_CLModel.continual_model_dcrnn import ContinualModel
...
def train_metr_la(args):
    logger = get_logger(args.logfile)
    device = torch.device(args.device)
    buffer = Buffer(256, device=device)
    
    _, _, _, scaler = generate_seq_dataset(file=args.data)
    bestid = 10000
    ...
    for task_id in range(metr_la_dataset.N_TASKS):
        logger.info('Task {} begins.'.format(task_id))

        batches_seen = 0

        train_loader, val_loader, test_loader, _ = metr_la_dataset.get_data_loaders(args)
        ...
        if task_id == 0:
            for i in range(1, args.epochs+1):
                ...
                for iter, (x, y) in enumerate(train_loader):
                    ...
                    trainx = x.permute(1, 0, 2, 3).float().to(device)
                    trainx = trainx.view(args.seq_length, args.batch_size, args.num_nodes*args.in_dim)
                    trainy = y.permute(1, 0, 2, 3).float().to(device)
                    trainy = trainy[..., :1].view(args.seq_length, args.batch_size, args.num_nodes*1)
                    ...
                    if buffer.is_empty():
                        ...
                        output, p1, p2, z1, z2 = model(trainx, trainy, batches_seen, aug1='edge', aug2='time')
                        if batches_seen == 0:
                            # this is a workaround to accommodate dynamically registered parameters in DCGRUCell
                            optimizer = torch.optim.Adam(model.parameters(), lr=args.learning_rate, eps=1e-8)
                        # output, p1, p2, z1, z2 = model(trainx, aug1=None, aug2='time')
                    else:
                        # trainx = F.pad(trainx, (1, 0, 0, 0))
                        # print('pad trainx: {}'.format(trainx.shape))
                        ...
                        output, p1, p2, z1, z2 = model(trainx, trainy, batches_seen, aug1='edge', aug2='time', lamda=lamda, buffer_x=buffer_x)
                        if batches_seen == 0:
                            # this is a workaround to accommodate dynamically registered parameters in DCGRUCell
                            optimizer = torch.optim.Adam(model.parameters(), lr=args.learning_rate, eps=1e-8)
                        # output, p1, p2, z1, z2 = model(trainx, aug1=None, aug2='time', lamda=lamda, buffer_x=buffer_x)
                ...
                for iter, (x, y) in enumerate(val_loader):
                    ...
                    testx = x.permute(1, 0, 2, 3).float().to(device)
                    testx = testx.view(args.seq_length, args.batch_size, args.num_nodes*args.in_dim)
                    testy = y.permute(1, 0, 2, 3).float().to(device)
                    testy = testy[..., :1].view(args.seq_length, args.batch_size, args.num_nodes*1)
            ...
            for i in range(1, args.epochs+1):
                ...
                for iter, (x, y) in enumerate(train_loader):
                    ...
                    # if buffer.is_empty():
                    #     trainx = F.pad(trainx, (1, 0, 0, 0))
                    #     output, p1, p2, z1, z2 = model(trainx, aug1='edge', aug2='time')
                    # else:
                    #     trainx = F.pad(trainx, (1, 0, 0, 0))
                    #     buffer_x = buffer.get_mir_data(model=model, size=128, batch_size=args.batch_size, lr=args.learning_rate)
                    #     lamda = np.random.beta(args.alpha, args.alpha)
                    #     output, p1, p2, z1, z2 = model(trainx, aug1='edge', aug2='time', lamda=lamda, buffer_x=buffer_x)
                    # output = output.transpose(1, 3)
                    # real = torch.unsqueeze(trainy[:, 0, :, :], dim=1)
                    # predict = scaler.inverse_transform(output)
                    # loss_mae = criteria_loss(predict, real, 0.0)
